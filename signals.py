...
def _classify_ride_structure_phase_info(
    *,
    ...
    Returns both a compatibility-safe route phase (used downstream) and a more
    expressive detail phase for alerts/diagnostics.
    """
    out: dict[str, object] = {
        'route_phase': 'UNSET',
        'detail_phase': 'UNSET',
    ...
    try:
        ...
        # Detail-phase taxonomy
        if failed:
            detail_phase = 'FAILED_EXTENSION'
            conf = float(np.clip(0.55 + 0.25 * reject + 0.20 * min(1.0, abs(dist_break)), 0.0, 0.98))
            interp = 'Continuation weakened after extension and lost control'
        elif compressed and defended and close_stack >= 0.55 and reject <= 0.34:
            detail_phase = 'BREAK_AND_HOLD'
            conf = float(np.clip(0.58 + 0.20 * close_stack + 0.12 * body_q + 0.10 * (1.10 - min(1.10, range_atr)), 0.0, 0.98))
            interp = 'Fresh breakout is holding above structure'
        elif defended and dist_break <= 0.32 and close_stack >= 0.58 and reject <= 0.34:
            detail_phase = 'EARLY_ACCEPTANCE'
            conf = float(np.clip(0.54 + 0.22 * close_stack + 0.12 * body_q + 0.10 * (0.34 - reject), 0.0, 0.97))
            interp = 'Price accepted above the key level and remains early in the move'
        elif defended and dist_break <= 0.95 and close_stack >= 0.72 and reject <= 0.24 and body_q >= 0.42:
            detail_phase = 'PERSISTENT_CONTINUATION'
            conf = float(np.clip(0.60 + 0.20 * close_stack + 0.12 * body_q + 0.10 * (0.24 - reject), 0.0, 0.99))
            interp = 'Trend is persistent and may not offer a full pullback'
        elif defended and dist_break <= 0.52 and close_stack >= 0.42 and reject <= 0.42:
            detail_phase = 'MATURE_ACCEPTANCE'
            conf = float(np.clip(0.48 + 0.18 * close_stack + 0.10 * body_q + 0.08 * (0.42 - reject), 0.0, 0.92))
            interp = 'Price is accepted above the key level but the move is more mature'
        elif dist_break >= 0.50:
            detail_phase = 'EXTEND_THEN_PULLBACK'
            conf = float(np.clip(0.52 + 0.16 * min(1.0, dist_break) + 0.12 * max(0.0, reject - 0.18) + 0.10 * max(0.0, 0.52 - body_q), 0.0, 0.96))
            interp = 'Trend remains healthy but current extension likely needs a reset'
        else:
            detail_phase = 'UNSTRUCTURED'
            conf = float(np.clip(0.40 + 0.12 * defended + 0.08 * close_stack, 0.0, 0.85))
            interp = 'No clean continuation phase established'
        ...
        out.update({
            'route_phase': route_map.get(detail_phase, 'UNSET'),
...
def compute_scalp_signal(
    ...
    *,
    ...
    ignition_long_strength += 1 if ((macd_build_long_pts >= 3) or macd_pre_long or macd_event) else 0
    ignition_long_strength += 1 if ignition_long_adx else 0
    ...
    ignition_short_strength += 1 if ((macd_build_short_pts >= 3) or macd_pre_short or macd_event) else 0
    ignition_short_strength += 1 if ignition_short_adx else 0
    ...
    primary_trigger = bool(vwap_event or rsi_event or macd_event or pro_trigger or early_pre_long or early_pre_short or ignition_override_long or ignition_override_short or tape_pre_long_assist or tape_pre_short_assist)
    ...
    pre_long_ok = bool(
        ...
        and isinstance(last_price, (float, int))
        and float(last_price) < float(ref_vwap)
        and (trigger_near or early_pre_long or ignition_override_long or tape_pre_long_assist)
        and (rsi_event or rsi_pre_long or macd_event or macd_pre_long or pro_trigger_long or ignition_override_long or (macd_build_long_pts >= 4))
        and (struct_pre_long or liquidity_event_long or orb_event_long or reclaim_attempt_long or ignition_override_long)
        and (long_confirmation_score >= max(2, effective_confirm_threshold - 1) or early_pre_long or ignition_override_long or tape_pre_long_assist)
    )
    pre_short_ok = bool(
        isinstance(ref_vwap, (float, int))
        and isinstance(last_price, (float, int))
        and float(last_price) > float(ref_vwap)
        and (trigger_near or early_pre_short or ignition_override_short or tape_pre_short_assist)
        and (rsi_event or rsi_pre_short or macd_event or macd_pre_short or pro_trigger_short or ignition_override_short or (macd_build_short_pts >= 4))
        and (struct_pre_short or liquidity_event_short or orb_event_short or reclaim_attempt_short or ignition_override_short)
        and (short_confirmation_score >= max(2, effective_confirm_threshold - 1) or early_pre_short or ignition_override_short or tape_pre_short_assist)
    )
    ...
    **_ignored: object,
    ...
    def _build_ride_entry_geometry(conviction_score: float) -> dict[str, object]:
        ...
        late_trend_shallow_bias_local = float(np.clip(
            0.42 * float(structure_phase_detail_local in ('EXTEND_THEN_PULLBACK', 'MATURE_ACCEPTANCE'))
        ...
        adaptive_pullback_refine_local = float(np.clip(
            0.34 * float(structure_phase_detail_local in ('MATURE_ACCEPTANCE', 'EXTEND_THEN_PULLBACK'))
        ...
        if structure_phase_detail_local == 'EARLY_ACCEPTANCE':
        ...
        elif structure_phase_detail_local == 'EXTEND_THEN_PULLBACK':
            phase_pullback_tighten_local = 0.08 * phase_tighten_conf_local
        elif structure_phase_detail_local == 'UNSTRUCTURED':
            phase_pullback_tighten_local = 0.0
        else:
            phase_pullback_tighten_local = 0.0
        ...
        if direction == "LONG":
            ...
            stop_mult_local = 0.55 if stage == "PRE" else 0.80
            stop_local = float(pullback_entry_local + stop_mult_local * atr_last)
        prefers_breakout_phase_local = bool(structure_phase_detail_local in ("BREAK_AND_HOLD", "EARLY_ACCEPTANCE", "PERSISTENT_CONTINUATION"))
        prefers_pullback_phase_local = bool(structure_phase_detail_local in ("EXTEND_THEN_PULLBACK", "FAILED_EXTENSION", "MATURE_ACCEPTANCE"))
        ...
        # Pre-break compression entry: when price is compressing tightly just under
        # the release line with rising directional pressure, allow RIDE to lean in
        # slightly before the actual breakout print. This helps catch the cleanest
        # continuation names that often do not offer a textbook breakout chase or
        # deep pullback after they coil.
        try:
            recent_pb_local = df.tail(int(min(4, len(df)))).copy()
            ...
            if min(len(pb_close_local), len(pb_open_local), len(pb_high_local), len(pb_low_local)) >= 4 and float(atr_last) > 0:
                ...
                pb_tight_local = bool(pb_range_atr_local <= 1.00 and pb_body_frac_local <= 0.68)
                if len(pb_vol_local) >= 4:
                    early_vol_local = float(np.mean(pb_vol_local.values[:2]))
                    late_vol_local = float(np.mean(pb_vol_local.values[-2:]))
                    pb_vol_tight_local = bool(late_vol_local <= 0.96 * max(1e-9, early_vol_local) and late_vol_local >= 0.60 * max(1e-9, float(med30 or 0.0)))
                else:
                    pb_vol_tight_local = bool(vol_side_q_local >= 0.45)
                if direction == 'LONG':
                    defended_pb_local = bool(float(pb_low_local.tail(int(min(3, len(pb_low_local)))).min()) >= float(accept_line) - 0.16 * float(atr_last))
                    trigger_ready_pb_local = bool(float(last_price) <= float(break_trigger_local) + 0.04 * float(atr_last) and float(last_price) >= float(break_trigger_local) - 0.42 * float(atr_last))
                    ...
                    wick_ok_pb_local = bool(wick_ratio_pb_local <= 0.30)
                else:
                    defended_pb_local = bool(float(pb_high_local.tail(int(min(3, len(pb_high_local)))).max()) <= float(accept_line) + 0.16 * float(atr_last))
                    trigger_ready_pb_local = bool(float(last_price) >= float(break_trigger_local) - 0.04 * float(atr_last) and float(last_price) <= float(break_trigger_local) + 0.42 * float(atr_last))
                    close_stack_pb_local = bool(float(pb_close_local.iloc[-1]) <= float(pb_close_local.iloc[-2]) + 0.03 * float(atr_last) and float(pb_close_local.iloc[-1]) <= float(pb_close_local.iloc[-3]) + 0.02 * float(atr_last))
                    ...
                    wick_ok_pb_local = bool(wick_ratio_pb_local <= 0.30)
                prebreak_compression_score_local = float(np.clip(
                ...
                prebreak_compression_ready_local = bool(
                    ...
                    and (not bool(multibar_extension_profile_local.get('stalling') or False))
                    and (not bool(multibar_extension_profile_local.get('fading') or multibar_extension_profile_local.get('momentum_fade') or False))
                    and prebreak_compression_score_local >= 0.66
                    and dist_br_local <= 1.28 * max(prox_dist_local, 0.32 * float(atr_last))
                )
        ...
        actionable_local = bool((near_pullback_local or near_break_local or prebreak_compression_ready_local or persistent_trend_ready_local) and not stale_breakout_local)
        ...
        try:
            ...
            elite_runaway_local = bool(
                ...
                and (structure_phase_detail_local in ("BREAK_AND_HOLD", "EARLY_ACCEPTANCE", "PERSISTENT_CONTINUATION"))
                and (not bool(multibar_extension_profile_local.get("path_stretched") or False))
                and (not bool(multibar_extension_profile_local.get("stalling") or False))
        ...
        if actionable_local:
            ...
            if tape_mode_enabled:
                ...
                try:
                    ...
                    standard_continuation_ok_local = bool(
                        ...
                        and (structure_phase_detail_local in ("BREAK_AND_HOLD", "EARLY_ACCEPTANCE", "PERSISTENT_CONTINUATION", "EXTEND_THEN_PULLBACK", "MATURE_ACCEPTANCE"))
                ...
                try:
                    ...
                    ignition_continuation_score_local = float(np.clip(
                        0.20 * float(structure_phase_detail_local == 'EARLY_ACCEPTANCE')
                        + 0.18 * float(np.clip(structure_phase_confidence_local, 0.0, 1.0))
                    ...
                    ignition_continuation_ready_local = bool(
                        ...
                        and (not near_pullback_local)
                        and (not stale_breakout_local)
                        and str(structure_phase_detail_local) == 'EARLY_ACCEPTANCE'
            ...
            breakout_fresh_phase_local = bool(str(structure_phase_detail_local) in ("BREAK_AND_HOLD", "EARLY_ACCEPTANCE", "PERSISTENT_CONTINUATION"))
            mature_compression_ready_local = bool(
                ...
                and float(close_stack_q_local) >= 0.52
                and float(persistence_q_local) >= 0.48
            ...
            phase_momentum_ready_local = bool(
                ...
                and breakout_proximity_ratio_local <= (1.24 if str(structure_phase_detail_local) == "PERSISTENT_CONTINUATION" else 1.18)
                and (
                    bool(ignition_continuation_ready_local)
                    or bool(persistent_trend_ready_local)
                    or bool(prebreak_compression_ready_local)
                    or (bool(breakout_bias) and bool(near_break_local) and float(persistence_q_local) >= 0.50 and float(close_stack_q_local) >= 0.50)
                )
            )
            ...
            if phase_no_trade_local:
                ...
                if phase_momentum_ready_local:
                    ...
                    if prebreak_compression_ready_local:
                        if direction == "LONG":
                            entry_base_local = float(np.clip(last_price, break_trigger_local - 0.16 * atr_last, break_trigger_local - 0.02 * atr_last))
                        else:
                            entry_base_local = float(np.clip(last_price, break_trigger_local + 0.02 * atr_last, break_trigger_local + 0.16 * atr_last))
    ...
    score = _cap_score(pts + ride_zone_adj + ride_cont_strength_adj + adx_modifier + provisional_cont_phase3_adj)
    ...
    score = _cap_score(pts + ride_zone_adj + ride_cont_strength_adj + adx_modifier + ride_cont_phase3_adj)
